...
fn load_builders(working_dir: &Option<String>) -> Result<HashMap<String, TransactionBuilder>, io::Error> {
...
fn status(args : StatusArgs) -> Result<(), io::Error> {
...
fn add(args: AddArgs) -> Result<(), io::Error> {
...
fn add_builder(builders: &mut HashMap<String, TransactionBuilder>, raw: Vec<u8>) -> Result<(),io::Error> {
...
fn prepare_zk_input(working_dir: &Option<String>) -> Result<(ZKPayload, HashMap<String, TXSet>), io::Error> {
    ...
    for (space, builder) in builders {
        ...
        let subtree = snapshot.prove_all(&keys).map_err(|e| {
...
fn prove(working_dir : &Option<String>) -> Result<(Vec<Commitment>, HashMap<String, TXSet>), io::Error> {
...
fn commit(args : CommitArgs) -> Result<(), io::Error> {
    let uncommitted_path = get_working_dir(&args.c)?.join(STAGING_FILE);
    if !std::path::Path::new(uncommitted_path.to_str().unwrap()).exists() {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "No changes to prove and commit"));
    }
